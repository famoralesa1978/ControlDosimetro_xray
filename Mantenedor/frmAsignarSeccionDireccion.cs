...
{
	public partial class frmAsignarSeccionDireccion : Form
	{
		...
		public frmAsignarSeccionDireccion(Int64 intCodCliente, string srtRun)
		{
			InitializeComponent();
			lblCantidad.Text = "";
			run = srtRun;
		...
		void GrabarSeccion()
		{
			...
			cmd.CommandText = String.Format("SeccionPersonalUpd {0},'{1}'",
																			cbxSeccion.SelectedValue, xmlSeccion());
		...
		private string xmlSucursal()
		{
			...
			foreach (object itemChecked in chkListaDir.CheckedItems)
			{
				DataRowView ItemFila = itemChecked as DataRowView;
				intId = (int)ItemFila["Id_personal"];
				strbSucursal.AppendFormat(@"<a e=""{0}""/>",
		...
		private void btnMarcarTodosDir_Click(object sender, EventArgs e)
		{
			for (int intFilaLista = 0; intFilaLista < chkListaDir.Items.Count; intFilaLista++)
			{
				var dr = ((DataRowView)chkListaDir.Items[intFilaLista]).Row;

				chkListaDir.SetItemChecked(intFilaLista, true);
			}
		}
		private void btnDesmarcarTodosDir_Click(object sender, EventArgs e)
		{
			for (int intFilaLista = 0; intFilaLista < chkListaDir.Items.Count; intFilaLista++)
			{
				var dr = ((DataRowView)chkListaDir.Items[intFilaLista]).Row;
				chkListaDir.SetItemChecked(intFilaLista, false);
			}
		}
		private void btnInvertirDir_Click(object sender, EventArgs e)
		{
			for (int intFilaLista = 0; intFilaLista < chkListaDir.Items.Count; intFilaLista++)
			{
				chkListaDir.SetItemChecked(intFilaLista, !chkListaDir.GetItemChecked(intFilaLista));
			}
